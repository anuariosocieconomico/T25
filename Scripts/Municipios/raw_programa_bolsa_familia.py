...
def create_payload(draw, start, length):
    return {
        "draw": str(draw),
        "columns[0][data]": "0",
        "columns[0][name]": "mes_ano_formatado",
        "columns[0][searchable]": "true",
        "columns[0][orderable]": "true",
        "columns[0][search][value]": "",
        "columns[0][search][regex]": "false",
        "columns[1][data]": "1",
        "columns[1][name]": "(case when t.mes_ano<='2021-10-01' then t.bf_qtd_fam else null",
        "columns[1][searchable]": "true",
        "columns[1][orderable]": "true",
        "columns[1][search][value]": "",
        "columns[1][search][regex]": "false",
        "columns[2][data]": "2",
        "columns[2][name]": "(case when t.mes_ano>='2023-03-01' then t.bf_qtd_fam else null",
        "columns[2][searchable]": "true",
        "columns[2][orderable]": "true",
        "columns[2][search][value]": "",
        "columns[2][search][regex]": "false",
        "columns[3][data]": "3",
        "columns[3][name]": "(case when t.mes_ano<='2021-10-01' then t.bf_vl else null end)",
        "columns[3][searchable]": "true",
        "columns[3][orderable]": "true",
        "columns[3][search][value]": "",
        "columns[3][search][regex]": "false",
        "columns[4][data]": "4",
        "columns[4][name]": "(case when t.mes_ano>='2023-03-01' then t.bf_vl else null end)",
        "columns[4][searchable]": "true",
        "columns[4][orderable]": "true",
        "columns[4][search][value]": "",
        "columns[4][search][regex]": "false",
        "columns[5][data]": "5",
        "columns[5][name]": "(case when t.mes_ano<='2021-10-01' then (case when t.bf_qtd_fam",
        "columns[5][searchable]": "true",
        "columns[5][orderable]": "true",
        "columns[5][search][value]": "",
        "columns[5][search][regex]": "false",
        "columns[6][data]": "6",
        "columns[6][name]": "(case when t.mes_ano>='2023-03-01' then (case when t.v2643>0 th",
...
column_names = [
    'Mês/Ano',
    'Famílias PBF até Out/21',
    'Famílias PBF a partir Mar/23',
    'Valor Repassado até Out/21',
    'Valor Repassado a partir Mar/23',
    'Valor Médio até Out/21',
    'Valor Médio a partir Mar/23'
]
data.columns = column_names

# Converte tipos de dados
data['Mês/Ano'] = data['Mês/Ano'].astype(str)
data['Famílias PBF até Out/21'] = pd.to_numeric(data['Famílias PBF até Out/21'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
data['Famílias PBF a partir Mar/23'] = pd.to_numeric(data['Famílias PBF a partir Mar/23'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
data['Valor Repassado até Out/21'] = pd.to_numeric(data['Valor Repassado até Out/21'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
data['Valor Repassado a partir Mar/23'] = pd.to_numeric(data['Valor Repassado a partir Mar/23'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
data['Valor Médio até Out/21'] = pd.to_numeric(data['Valor Médio até Out/21'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
data['Valor Médio a partir Mar/23'] = pd.to_numeric(data['Valor Médio a partir Mar/23'].astype(str).str.replace('.', '').str.replace(',', '.').str.replace('-', '').str.replace('R$', '').str.strip(), errors='coerce')
