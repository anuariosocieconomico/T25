...
db_name = 'pesquisa_pecuaria_municipal_aquicultura_ppm'  # nome base da base de dados; alterar conforme necessário e o valor será atualizado em todo o script
...
try:
    ...
    for year in range(2010, datetime.now().year + 1):
        ...
        try:
            ...
            specific_columns = ['Tipo de produto da aquicultura']
    ...
    with open(os.path.join(error_path, 'log_raw_pesquisa_pecuaria_municipal_aquicultura_ppm.txt'), 'w', encoding='utf-8') as f:
        f.write(f'Erro em raw_pesquisa_pecuaria_municipal_aquicultura_ppm.py em {pd.Timestamp.now().strftime("%d/%m/%Y %H:%M:%S")}\n')
        f.write(error)
    print('Erro ao baixar ou processar os dados. Verifique o log em "Doc/Municipios/log_raw_pesquisa_pecuaria_municipal_aquicultura_ppm.txt".')
