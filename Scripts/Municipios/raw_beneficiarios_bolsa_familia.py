...
try:
    ...
    with open(os.path.join(error_path, 'log_raw_beneficiarios_bolsa_familia.txt'), 'w', encoding='utf-8') as f:
        f.write(f'Erro em raw_beneficiarios_bolsa_familia.py em {pd.Timestamp.now().strftime("%d/%m/%Y %H:%M:%S")}\n')
        f.write(error)
    print('Erro ao baixar ou processar os dados. Verifique o log em "Doc/Municipios/log_raw_beneficiarios_bolsa_familia.txt".')