...
db_name = 'producao_extracao_vegetal_silvicultra_extracao_pev'  # nome base da base de dados; alterar conforme necessário e o valor será atualizado em todo o script
...
try:
    ...
    for year in range(2010, datetime.now().year + 1):
        ...
        try:
            ...
            url = f'https://apisidra.ibge.gov.br/values/t/289/n6/2801207,2802403,2804201,2804508,2805406,2805604,2800100,2800704,2801108,2802700,2804409,2804706,2804904,2805703,2807303,2800209,2801405,2801603,2804458,2801900,2802205,2802304,2802601,2803104,2803401,2803807,2804300,2804607,2805000,2805208,2806008,2806909,2807006,2801306,2802007,2803302,2804003,2805307,2805901,2806107,2806602,2807204,2800506,2801009,2802908,2803708,2803906,2804102,2806800,2800407,2800670,2803005,2803500,2805109,2805802,2806206,2807105,2800308,2800605,2802106,2802809,2803203,2806305,2806701,2807600,2801702,2805505,2807402,2807501,2806404/v/allxp/p/{year}/c193/allxt'
            ...
            specific_columns = ['Tipo de produto extrativo']
    ...
    with open(os.path.join(error_path, 'log_raw_producao_extracao_vegetal_silvicultra_extracao_pev.txt'), 'w', encoding='utf-8') as f:
        f.write(f'Erro em raw_producao_extracao_vegetal_silvicultra_extracao_pev.py em {pd.Timestamp.now().strftime("%d/%m/%Y %H:%M:%S")}\n')
        f.write(error)
    print('Erro ao baixar ou processar os dados. Verifique o log em "Doc/Municipios/log_raw_producao_extracao_vegetal_silvicultra_extracao_pev.txt".')